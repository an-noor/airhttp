package com.airhttp.context
{
	...
	public class HttpRequest extends EventDispatcher
	{
		...
		protected var sr:HttpRequestReader;
				...
				if(line!=""){
					...
					switch(contentType){
						...
						case APPLICATION_JSON:
							_buffer.clear();
							sr.byteReaded=0;
							socket.addEventListener(ProgressEvent.SOCKET_DATA,reader_json);
							reader_json();
							break;
		 ...
		 */		
		private function reader_json(...args):void{
			this.reset_clock();
			sr.readLine();
			if (sr.byteReaded >= contentLength) {
				socket.removeEventListener(ProgressEvent.SOCKET_DATA, reader_json);
				json = JSON.parse(sr.buffer.toString());
				reader_end();
			}
		}
